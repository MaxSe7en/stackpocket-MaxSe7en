...
import { trackReducer, cartReducer } from "@/games/5d/useReducer";
import {
  ACTION_TYPES,
  INITIAL_TRACK_STATE,
  TRACK_ACTION_TYPES,
...
import TwoSide from "../2sides/index";
...
const GamePlayWrapper: React.FC<AppProps> = ({
  ...
  const {} = generateNumbers(start, end, dispatch);
  ...
  const Buttons2 = ["Big", "Small", "Odd", "Even"];
  ...
  const modifiedBet = useMemo(() => {
    switch (gameGroup) {
      ...
      case "Dragon/Tiger/Tie":
        ...
        return {
          ...bet,
          allSelections: [filterEmptyValues(selections)],
          userSelections: filterDragonTieThreeCardsSelections(u),
        };
      case "Three cards":
        let user_selections = filterObjectSelections(selections);
        return {
          ...bet,
          allSelections: filterEmptyValues(selections),
          userSelections: filterDragonTieThreeCardsSelections(user_selections),
        };
      case "B/S/O/E":
        return {
          ...bet,
          userSelections: concatenateArrays(
            studBullSelections(selections, Buttons2)
          ),
        };
      case "Stud":
        return {
          ...bet,
          userSelections: studBullSelections(selections, Buttons)?.join(", "),
        };
      case "Bull Bull":
        return {
          ...bet,
          userSelections: studBullSelections(selections, Buttons1)?.join(", "),
        };
      case "Pick 2":
      case "Pick 3":
      case "Pick 4":
        let pickJointGames = [125, 132, 139];
        return {
          ...bet,
          allSelections: pickJointGames.includes(gameType_id)
            ? Object.values(selections)
            : [checkedBoxes, filterSelections(selections)[0]],
          userSelections: pickJointGames.includes(gameType_id)
            ? Object.values(selections)
                .map((item: any) => (item?.length === 0 ? "-" : item))
                .join(",")
            : `(${pickGamesSelections(checkedBoxes)}), ${filterSelections(
                selections
              )}`,
        };
      default:
  ...
  const manual: Readonly<number>[] = [
    2, 11, 18, 25, 31, 33, 34, 38, 44, 46, 47, 51, 57, 59, 60, 64, 68, 92, 96,
    126, 129, 133, 137, 140,
  ];
  ...
  const handleModalContinue = async () => {
    handleCloseModal();
    console.log("handleModalContinue: ");
  };
  ...
  return (
    <>
      ...
      <div className={styles.board}>
        ...
        {gameButtonType === "manual" && gameModels === "standard" ? (
          <Manual
        ...
        {gameModels === "twosides" ? <TwoSide /> : null}
              ...
              <div className={styles.plate}>
                <center>
                  ...
                  <button
                    ...
                    onClick={handleCartBetNowConfirm}
                    // onClick={() =>
                    //   cartBetNow(
                    //     cartState,
                    //     betId,
