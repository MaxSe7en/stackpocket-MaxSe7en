...
CoinStackBar.prototype.setDimensions = function (containerwidth, containerheight) {
	...
	let widthsize = containerwidth / this.coinimgwidth;
	let heightsize = containerheight / (this.coinimgheight + this.maxstackheight * this.coinheight);
...
CoinStackBar.prototype.setValue = function (value, newmin, newmax) {
	...
	let coins = Math.round((clamped / this.max) * this.maxstackheight);
	//Also calculate the amount of coins that were already in the stack.
	let oldcoins = Math.round((this.value / this.max) * this.maxstackheight);
...
CoinStackBar.prototype.createNewCoin = function (i, durationoffset) {
	...
	let coinelem = document.createElement('img');

	//If array, pick an image at random.
	if (Object.prototype.toString.call(this.coinimgsrc) === '[object Array]') {
		let index = Math.floor(Math.abs(this.rand(i + 1, this.seed) * this.coinimgsrc.length));
	...
	let coinxpos = +this.rand(i, this.seed) * (this.xoffset * this.stacksize);
	let coinypos = i * (this.coinheight * this.stacksize) + this.rand(i, this.seed) * (this.yoffset * this.stacksize);
...
CoinStackBar.prototype.removeCoin = function (i) {
	let coins = this.container.getElementsByTagName("img");
	if (coins[i]) {
...
CoinStackBar.prototype.update = function () {
	...
	if (this.coins) {
		...
		this.coins.forEach(function (coin, key, CoinStackBar) {
			...
			let animframe = 1.0 - ((coin.starttime + coin.duration) - Date.now()) / coin.duration;
